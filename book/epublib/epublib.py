...
class EpubBook():
    ...
    def pretext(self, txt_name):
        '''
            txt_name: txt文件名字
        '''
        if os.path.splitext(txt_name)[-1] != '.txt':
            print("Is not '.txt'")
            return
    ...
    def init_epub(self):
        ...
        with open(op + 'OEBPS/content.opf', "w", encoding='utf8') as a:
            a.writelines(content % (self.book_name, self.author))
        with open(op + 'OEBPS/toc.ncx', "w", encoding='utf8') as a:
            a.writelines(toc % (self.book_name))
    ...
    def add_content(self, lis_):
        ...
        for name in lis_:
            dic = {
                'href': name,
                'id': 'id' + re.search(r'\d+',name).group(),
    ...
    def add_toc(self, dic):
        ...
        self.conc_con_toc(dic.keys())
        with open(self.epub_source + 'OEBPS/toc.ncx', "r", encoding='utf8') as a:
            text = a.read()
        bf = BeautifulSoup(text, 'lxml')
        tag = bf.find('navmap')
        for name, title in dic.items():
            new_tag1 = bf.new_tag('navPoint', _class='chapter', id='id'+re.search(
            ...
            tag.append(new_tag1)
        with open(self.epub_source + 'OEBPS/toc.ncx', "w", encoding='utf8') as a:
        ...
        for i in os.listdir(self.txt_input):
            if os.path.splitext(i)[-1] == '.txt':
                if not lis_.count(i.replace('txt', 'html')):
                    dic_[i.replace('txt', 'html')] = self.pretext(i)
        self.add_toc(dic_)
        print("Update done ! ")
